import type { StrikeKind } from '../../../domain/StrikeEvent'
...
const KIND_COLOR: Record<StrikeKind, string> = {
  muletilla: 'bg-warning',
  accentuation: 'bg-accent',
  pronunciation: 'bg-danger',
}
...
// passes the 44x44 touch target rule when scaled with the parent (see
// CSS comment).
export const StrikeMarker = ({ positionPct, kind, onClick, label }: StrikeMarkerProps) => {
  ...
  return (
    <button
      ...
      className={`absolute -top-2 -translate-x-1/2 h-3 w-3 rounded-full border-2 border-bg ${KIND_COLOR[kind]}
